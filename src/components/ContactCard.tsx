      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasSurinameRelevance && (
              <span className="shrink-0 rounded-full bg-[#1a6e74]/10 px-2 py-0.5 text-[11px] font-medium text-[#1a6e74]">
                Suriname
              </span>
            )}
          </div>
          ...
          {contact.labels.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {contact.labels.map((label, i) => (
                <span
                  key={i}
                  className="inline-flex items-center rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground transition-colors group-hover:bg-gold/10 group-hover:text-gold-dark"
                >
                  {label}
                </span>
              ))}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-border/60 pt-3" onClick={(e) => e.stopPropagation()}>
            ...
            {contact.linkedin_url && (
              <a href={contact.linkedin_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-[#0077B5]/10 hover:text-[#0077B5]" data-testid={`link-linkedin-${contact.id}`}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" className="shrink-0"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
                <span>LinkedIn</span>
              </a>
            )}
          </div>
